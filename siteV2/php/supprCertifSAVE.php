...
<?php
   ...
   if(empty($_SESSION['id'])){
	...
	$sqlDel="DELETE FROM avoir WHERE IdCertif=? AND IdAdh=? AND IdProducteur=? ";


	$prepareDel=mysqli_prepare($link,$sqlDel);
	mysqli_stmt_bind_param($prepareDel,'sss',$_GET['certif'],$_GET['adh'],$_GET['prod']);
...
<br><br><br><b>  <br><br> 
<?php
if(mysqli_stmt_execute($prepareDel)){
		echo utf8_encode("Tout s'est bien passé, la certification a été supprimé.... ");
	}
	else{
		echo "Erreur.... ";
	}
